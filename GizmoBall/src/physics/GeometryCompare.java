 ...
 * the same behavior.
 * GeometryCompare 是Geometry 接口的实现，它由另外两个具体的实现组成，并且检查他们有相同的行为。
  ...
  implements GeometryInterface
{
   ...
   * the result from g1.
   * 作用：返回一个新的GeometryCompare 分派给g1 and g2, 如果答案不同则抛出异常
   * 并返回g1的结果。
   **/
   ...
   * 当一些字符窜不常使用时，字符串thunk是延缓他们的生成的一种方式。
   * 比如，一个断言的失败信息很少被使用，只当断言失败时才被使用。
   **/
  ...
  /**
   * A simple empty-string thunk. 简单的空字符串thunk
   **/
   ...
   * 一种方便的方法来预计或添加String 到StringThunk
   **/
   ...
   *         message
   * 抛出一个RuntimeException ，这个异常的消息包括2个值的信息，他们的区别，和提供的消息
   **/
   ...
   * 作用： 返还a如果ab的component 在他们的公差范围内，否则抛出异常
   **/
