...
public class TrappingRainWater {
	...
	public static Integer solution(Integer[] intArray) {
		Integer result = 0; 
		Integer hightsOfBlocks = 0; //各ブロックの高さ、つまり、intArray[i]の値；
		//以下はintArrayのindexを表示してくれるものを作るため、intArrayと同じ長さのInteger型Array；
		Integer index = 0;
		Integer[] indexArray = new Integer[intArray.length];
		for(int i=0; i<intArray.length; i++) {
			indexArray[i] = i;
		}
		//以上はintArrayのindexを表示してくれるものを作るため、intArrayと同じ長さのInteger型Array；
		Integer indexOfMax1 =indexOfMax(intArray);//indexOfMaxメソードを使って、intArrayの中のmaxのindexをリラーンする；
		if(indexOfMax1!=0) { //intArray[0]がmaxかどうかを判断する；maxじゃない場合、maxが出てくるまで、右が自分より低い場合は、ずっと右を見ていく；
			Integer leftBlock = intArray[0];
			for(int i=1; i<intArray.length; i++) {
				if(intArray[i] > leftBlock) {
					result = result + (i-index-1)*leftBlock-hightsOfBlocks;//右が自分より高い場合、つまり、水を貯められるということ、なのでどれだけ貯まるかを算出；
					index = i;
					hightsOfBlocks = 0;
					if(indexOfMax1==i) {
						break;//ここでmaxが出てきたため、これからの右は全部このmaxより低いため、処理法が変わる；なので、if文を出て、次のwhileループに入る；
					}else {
		...
		//ここからは、maxの右側について処理する；
		while(index<intArray.length-1) {	
			//まずはこの部分（新しいArrayを作って、currentのmaxの右側を全部これに入れる；目的は、この中から一番高いブロック（次のmax）を探すこと；
			Integer[] subArray = new Integer[(intArray.length-1) - index];
			for(int i=index+1; i<intArray.length; i++) {
				subArray[i-(index+1)] = intArray[i];
			}
			//以上で、新しいArrayを作って、currentのmaxの右側を全部それに入れた；
			Integer indexOfMax2 = indexOfMax(subArray);//この中から一番高いブロック（次のmax）を探しだす；
			if(indexOfMax2==0) {//maxのindexが０だというのは、前のmaxの次にすぐ次のmaxであること；この場合、この二つのブロックの間に見ず溜まらない；
				index=index+1;
			}else {//前のmaxと次のmaxの間に、水が溜まっているので、これを算出する；
